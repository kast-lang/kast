...
impl CanvasRenderingContext as module = (
    ...
    const strokeRect = (x, y, width, height) => (
        ctx => (
            (@native "async ({ctx,x,y,width,height}) => {ctx.strokeRect(x,y,width,height)}") (
                .ctx = ctx.raw, .x, .y, .width, .height
            )
        )
    );
    const fillRect = (x, y, width, height) => (
        ctx => (
            (@native "async ({ctx,x,y,width,height}) => {ctx.fillRect(x,y,width,height)}") (
                .ctx = ctx.raw, .x, .y, .width, .height
            )
        )
    );
    const beginPath = (ctx) => (
        (@native "async ({ctx}) => {ctx.beginPath()}") (
            .ctx = ctx.raw
        )
    );
    const closePath = (ctx) => (
        (@native "async ({ctx}) => {ctx.closePath()}") (
            .ctx = ctx.raw
        )
    );
    const moveTo = (x, y) => (
        (ctx) => (
            (@native "async ({ctx,x,y}) => {ctx.moveTo(x,y)}") (
                .ctx = ctx.raw, .x, .y
            )
        )
    );
    const lineTo = (x, y) => (
        (ctx) => (
            (@native "async ({ctx,x,y}) => {ctx.lineTo(x,y)}") (
                .ctx = ctx.raw, .x, .y
            )
        )
    );
    const stroke = (ctx) => (
        (@native "async ({ctx}) => {ctx.stroke()}") (
            .ctx = ctx.raw
        )
    );
);
...
const createElement = (document, t) => (
    (@native "async ({document, t}) => document.createElement(t)") (
        .document, .t
    )
);
const appendChild = (element, child) => (
    (@native "async ({element, child}) => element.appendChild(child)") (
        .element, .child
    )
);
...
const getContext = (canvas, t) -> CanvasRenderingContext => (
    .raw = (@native "async ({canvas, t}) => canvas.getContext(t)") (
        .canvas, .t
    ),
);
