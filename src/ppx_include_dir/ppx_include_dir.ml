...
let get_blob ~loc path =
  ...
  | Some blob -> blob
  | None -> location_errorf ~loc "[%%include_dir] could not find or load file %s" path
;;
...
let rec get_entries ~loc path =
    ...
    let entries =
      ...
      |> List.map (fun entry ->
        ...
        let entry_expr : expression =
          ...
          | false ->
            let contents = Ast_builder.Default.estring ~loc (get_blob ~loc entry_path) in
            let file =
  ...
  with
  | _ -> location_errorf ~loc "[%%include_dir] could not find or load dir %s" path
...
and include_dir ~loc path =
  ...
  in
  let entries =
    Ast_builder.Default.pexp_apply ~loc string_map_of_list [ Nolabel, entries ]
  in
  let expr =
    Ast_builder.Default.pexp_record ~loc [ { txt = Lident "entries"; loc }, entries ] None
  in
  let ty : core_type =
