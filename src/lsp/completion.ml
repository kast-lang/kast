...
let rec complete_from_compiler_scope (scope : Types.compiler_scope) :
  ...
  let locals =
    ...
    |> StringMap.map (fun (binding : binding) ->
           ...
           let detail = make_string "@[<h>%a@]" Ty.print binding.ty in
...
let rec complete :
    ...
    position ->
    check_touch:bool ->
    Lsp.Types.CompletionItem.t list option =
  ...
  if (not check_touch) || data.span |> Span.touches pos then (
    ...
    let inner =
      ...
      |> Seq.find_map (fun (Common.CompiledThing (kind, compiled)) ->
             complete kind compiled pos ~check_touch:true)
    in
    match inner with
...
let completions (pos : Lsp.Types.Position.t)
  ...
  match compiled with
  | Some expr -> (
      match complete Expr expr pos ~check_touch:false with
      | Some list -> list
      | None -> [])
  | None -> []
