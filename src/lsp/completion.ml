    ...
    |> StringMap.map (fun local ->
      ...
      let kind : Lsp.Types.CompletionItemKind.t =
        ...
        | Some ty ->
          (match ty with
           | Types.T_Unit -> Unit
           | Types.T_Ast | Types.T_UnwindToken _ | Types.T_Error | Types.T_Ref _
           | Types.T_Bool | Types.T_Int32 | Types.T_UInt32 | Types.T_Int64
           | Types.T_UInt64 | Types.T_Float32 | Types.T_Float64 | Types.T_Char
           | Types.T_String | Types.T_Blocked _ | Types.T_Tuple _ | Types.T_List _
           | Types.T_Target | Types.T_Opaque _ | Types.T_Variant _ ->
             Variable
...
let rec find_expr : 'a. 'a compiled_kind -> 'a -> position -> compiled option =
  ...
  if Position.compare data.span.finish pos <= 0
  then Some (Types.Compiled (kind, compiled))
  else if data.span |> Span.contains_position pos
  then (
    let inner =
...
let completions (pos : Lsp.Types.Position.t) ({ compiled; _ } : Processing.file_state)
