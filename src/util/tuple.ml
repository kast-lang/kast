...
module Tuple = struct
  type 'a t =
    { unnamed : 'a array
    ; named : 'a StringMap.t
    ; named_order_rev : string list [@equal fun _ _ -> true] [@compare fun _ _ -> 0]
    }
  [@@deriving eq, ord]
  ...
  let is_unnamed len tuple =
    tuple.named_order_rev |> List.length = 0 && tuple.unnamed |> Array.length = len
  ;;

  let zip_order_a : 'a 'b. 'a tuple -> 'b tuple -> ('a * 'b) tuple =
    fun a b ->
    let unnamed =
      try Array.combine a.unnamed b.unnamed with
      | Invalid_argument _ -> invalid_arg "Tuple.zip (unnamed)"
    in
  ...
  let iter : 'a. (member -> 'a -> unit) -> 'a tuple -> unit =
    fun f tuple -> to_seq tuple |> Seq.iter (fun (member, value) -> f member value)
  ;;
  ...
  let add : 'a. string option -> 'a -> 'a tuple -> 'a tuple =
    fun (type a) (name : string option) (value : a) (tuple : a tuple) : a tuple ->
    match name with
    | Some name ->
      let name_was_present = StringMap.find_opt name tuple.named |> Option.is_some in
      { tuple with
        named = StringMap.add name value tuple.named
      ; named_order_rev =
          (if name_was_present
           then tuple.named_order_rev
           else name :: tuple.named_order_rev)
      }
    | None -> { tuple with unnamed = Array.append tuple.unnamed (Array.make 1 value) }
  ;;
  ...
  let merge : 'a. 'a tuple -> 'a tuple -> 'a tuple =
    ...
    let named =
      StringMap.union (fun _name _a _b -> invalid_arg "Tuple.merge") a.named b.named
    in
    { unnamed; named; named_order_rev }
  ;;
  ...
  let print
    ...
    -> unit
    =
    fun ?(options = default_print_options)
      print_value
      fmt
      { unnamed; named; named_order_rev } ->
    ...
    let print_named fmt name =
      ...
      fprintf
        fmt
        "@[<v>%s%a%s%a@]"
        options.named_field_before
        String.print_maybe_escaped
        name
        options.named_field_middle
        print_value
        value
    in
  ...
  let unwrap : 'a. unnamed:int -> named:string list -> 'a tuple -> 'a list =
    ...
    then fail "Expected %d unnamed fields, got %d" unnamed actual_unnamed;
    let named_set = StringSet.of_list named in
    let actual_unnamed = tuple.named_order_rev |> StringSet.of_list in
    if StringSet.equal named_set actual_unnamed |> not
    then
      fail
