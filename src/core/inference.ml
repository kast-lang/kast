...
module Var = struct
   ...
   fun unite_inferred ~span a b ->
    try
      ...
      if root_a == root_b then a
        ...
        root_a
    with effect (Error.T _ as eff), _k ->
      Effect.perform eff;
      a
  ...
  let infer_as : 'a. 'a unite -> span:span -> 'a -> 'a var -> unit =
   fun unite_inferred ~span infer_as var ->
    ...
    with effect (Error.T _ as eff), _k -> Effect.perform eff

  let infer_same_as : 'a. 'a unite -> span:span -> 'a var -> 'a var -> unit =
   fun unite_inferred ~span expected var ->
    unite unite_inferred ~span expected var |> ignore
