...
let init () =
  ...
  let exec =
    native_fn "sys.exec" (fun _ty ~caller ~state:_ arg : value ->
        match arg |> Value.await_inferred with
        | V_String cmd ->
            V_Int32 (Stdlib.Sys.command cmd |> Int32.of_int)
            |> Value.inferred ~span
        | _ ->
  ...
  let get_env =
    native_fn "sys.get_env" (fun ty ~caller ~state:_ arg : value ->
        ...
        let label_found, xs =
          match
            Inference.Var.inferred_opt sum_variant_ty.variants.var |> Option.get
          with
          | R_Cons { label; rest; _ } ->
        ...
        | V_String var -> (
            let env_val =
              try Some (Stdlib.Sys.getenv var) with Not_found -> None
            in
            match env_val with
            | Some s ->
                V_Variant
            ...
            | None ->
                V_Variant
                  { label = label_not_found; data = None; ty = sum_variant_ty }
                |> Value.inferred ~span)
        | _ ->
            Error.error caller "sys.exec expected string arg";
            V_Error |> Value.inferred ~span)
