...
let tcp () =
      ...
      | V_Ref { mut = _; place } ->
        (match read_place ~span place |> Value.await_inferred with
         | V_Opaque { ty = _; value = stream } ->
           ...
           let result = ref (Bytes.sub_string stream.buffer 0 stream.buf_pos) in
           let rec read_loop () =
             match String.index_opt !result '\n' with
             | Some newline_idx ->
               let after_newline = String.length !result - newline_idx - 1 in
               Bytes.blit_string !result (newline_idx + 1) stream.buffer 0 after_newline;
             ...
             | None ->
               ...
               let read = Bytes.sub_string stream.buffer stream.buf_pos bytes_read in
               result := !result ^ read;
               stream.buf_pos <- 0;
               read_loop ()
           in
      ...
      | T_Opaque result_ty ->
        (match arg |> Value.await_inferred with
         | V_String uri ->
           ...
           in
           let stream : tcp_stream =
             { file = Unix.socket addr.ai_family addr.ai_socktype addr.ai_protocol
             ; buffer = Bytes.create 4096
             ; buf_pos = 0
             }
           in
           Unix.connect stream.file addr.ai_addr;
           V_Opaque { ty = result_ty; value = Obj.repr stream } |> Value.inferred ~span
         | _ ->
      ...
      | T_Opaque result_ty ->
        (match arg |> Value.await_inferred with
         | V_String uri ->
           ...
           V_Opaque { ty = result_ty; value = Obj.repr listener } |> Value.inferred ~span
         | _ ->
  ...
  ; native_fn "net.tcp.accept" (fun ty ~caller ~state:_ args : value ->
      ...
      | T_Tuple ({ tuple = result_ty_field; _ } as result_ty) ->
        ...
        in
        (match stream_ty.ty |> Ty.await_inferred, addr_ty.ty |> Ty.await_inferred with
         ...
         | T_Opaque client_stream_ty, T_String ->
           ...
           in
           let close_on_exec =
             close_on_exec.place |> claim ~span |> Value.expect_bool |> Option.get
           in
           (* Native call *)
           ...
           let client_stream : tcp_stream =
             { file = client_stream; buffer = Bytes.create 4096; buf_pos = 0 }
           in
           let client_addr =
           ...
           let client_stream_value : Types.value_tuple_field =
             { place =
                 V_Opaque { ty = client_stream_ty; value = Obj.repr client_stream }
                 |> Value.inferred ~span
           ...
           let client_addr_value : Types.value_tuple_field =
             { place =
                 V_String client_addr |> Value.inferred ~span |> Place.init ~mut:Inherit
             ; span
             ; ty_field = addr_ty
             }
           in
           V_Tuple
             ...
             ; tuple =
                 Tuple.of_list
                   [ Some "stream", client_stream_value; Some "addr", client_addr_value ]
             }
           |> Value.inferred ~span
         | _ ->
           Error.error
  ...
  ; native_fn "net.tcp.listener.close" (fun _ty ~caller ~state:_ args : value ->
      ...
      | V_Opaque { ty = _; value = stream } ->
        ...
        V_Unit |> Value.inferred ~span
      | _ ->
        Error.error caller "net.tcp.listener.close expected the tcp.listener";
        V_Error |> Value.inferred ~span)
  ]
;;
