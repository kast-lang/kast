...
module Var = struct
  ...
  let new_not_inferred :
      'a 'scope. span:span -> scope:'scope -> ('a, 'scope) var =
   fun ~span ~scope ->
    {
      state =
        Root
  ...
  let new_inferred :
      'a 'scope. ('a -> 'scope) -> span:span -> 'a -> ('a, 'scope) var =
  ...
  let print :
      'a 'scope.
      (formatter -> 'a -> unit) -> formatter -> ('a, 'scope) var -> unit =
  ...
  let unite_data =
   fun ~span unite_inferred unite_scope
       ({
  ...
  let unite :
      'a 'scope.
      'a unite -> ('scope -> 'scope -> 'scope) -> ('a, 'scope) var unite =
   fun unite_inferred unite_scope ~span a b ->
    try
      ...
      if root_a == root_b then a
        ...
        let united_data =
          unite_data ~span unite_inferred unite_scope data_a data_b
        in
  ...
  let infer_as :
   ...
   fun inferred_scope unite_inferred unite_scope ~span infer_as var ->
    unite unite_inferred unite_scope ~span
