...
let new_not_inferred ~scope ~span = { var = Inference.Var.new_not_inferred ~scope ~span }
...
let inferred
      ...
      shape
  =
  { var = Inference.Var.new_inferred (shape_scope (module Scope) value_scope) ~span shape
  }
;;
...
let rec unite_shape
  ...
  | R_Empty, R_Cons _ | R_Cons _, R_Empty ->
    ...
    R_Empty
  | R_Cons a, R_Cons b ->
    if Label.get_name a.label = Label.get_name b.label
    then
      R_Cons
        { label = Label.unite a.label b.label
        ; value = unite_value ~span a.value b.value
        ; rest = unite (module Scope) value_scope unite_value ~span a.rest b.rest
        }
    else (
      let rest =
        new_not_inferred ~span ~scope:(Scope.deepest (scope a.rest) (scope b.rest))
      in
      a.rest.var
...
let rec of_list
  ...
  | (label, value) :: rest ->
    ...
    <| R_Cons { label; value; rest = of_list (module Scope) value_scope ~span rest }
;;
