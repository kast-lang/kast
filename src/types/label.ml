...
let equal a b =
  Inference.CompareRecurseCache.with_cache
    ...
    (fun () -> Inference.equal_var equal_label_data NoScope.equal a.var b.var)
;;

let compare a b = Inference.compare_var compare_label_data NoScope.compare a.var b.var
...
let get_data_span data =
  data.definition |> Option.unwrap_or_else (fun () -> data.references |> List.head)
;;

let get_span { var } = var |> Inference.Var.inferred_opt |> Option.get |> get_data_span
...
let unite =
  ...
  fun a b -> { var = Inference.Var.unite unite_data NoScope.unite ~span a.var b.var }
;;
